...
import 'package:focus_assist_chat_app/Widgets/full_photo_widget.dart';
...
class Chat extends StatelessWidget {
  ...
  Chat({Key key, @required this.userChat, @required this.currentUserChat}) : super(key: key);
...
class ChatScreen extends StatefulWidget {
  ...
  ChatScreen({Key key, @required this.userChat, @required this.currentUserChat}) : super(key: key);

  @override
  State createState() => ChatScreenState(userChat: userChat, currentUserChat: currentUserChat);
}

class ChatScreenState extends State<ChatScreen> {
  ChatScreenState({Key key, @required this.userChat, @required this.currentUserChat});
  ...
  readLocal() async {
    ...
    FirebaseFirestore.instance.collection('users').doc(currentUserChat.id).update({'chattingWith': userChat.id});
  ...
  bool isLastMessageLeft(int index) {
    if ((index > 0 && listMessage[index - 1].get('idFrom') == currentUserChat.id) || index == 0) {
      return true;
    } else {
      return false;
    }
  ...
  Future<bool> onBackPress() {
    if (isShowSticker) {
      ...
      FirebaseFirestore.instance.collection('users').doc(currentUserChat.id).update({'chattingWith': null});
